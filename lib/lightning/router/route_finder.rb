...
module Lightning
  module Router
    module RouteFinder
      ...
      # @param channels Array of short_channel_id
      # @return
      def self.find(source, target, updates, _assisted_routes, channels)
        routes = []
        if channels.empty?
          ...
          channels.map do |short_channel_id|
        ...
        updates = updates.select { |k, _v| channels.include?(k[:id]) } unless channels.empty?
