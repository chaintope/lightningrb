      ...
      class AcceptChannel < Lightning::Wire::LightningMessages::Generated::AcceptChannel
        ...
        def validate!(open)
          ...
          if self[:channel_reserve_satoshis] < open[:dust_limit_satoshis]
            raise Lightning::Exceptions::InsufficientChannelReserve.new(self.temporary_channel_id, self[:channel_reserve_satoshis])
          end
          if self[:dust_limit_satoshis] > open[:channel_reserve_satoshis]
            raise Lightning::Exceptions::InsufficientChannelReserve.new(self.temporary_channel_id, open[:channel_reserve_satoshis])
          end
        end
      end
    end
